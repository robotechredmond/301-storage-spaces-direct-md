...
configuration ConfigS2D
{
    ...
    Node localhost
    {
        ...
        {
            SetScript = "Add-CauClusterRole -ClusterName ${ClusterName} -CauPluginName Microsoft.WindowsUpdatePlugin -CauPluginArguments @{'IncludeRecommendedUpdates'='True'} -RequireAllNodesOnline -EnableFirewallRules -StartDate ${scheduledUpdateTime} -DaysOfWeek ${scheduledUpdateDay} -WeeksInterval 1 -Force"
